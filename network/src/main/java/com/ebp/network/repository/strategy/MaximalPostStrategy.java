...
public class MaximalPostStrategy implements CallStrategy {

    @Override
    public Single<ResponseBody> call(ServiceResources resources, ResourceConfiguration configuration) {
        return resources.post(configuration.getResource(), configuration.getHeaders(), configuration.getBody());
    }
}
